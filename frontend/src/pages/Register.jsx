    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        ...
        <div className="card p-7">
          <h1 className="font-display text-xl font-semibold text-stone-900 text-center">Gabung dengan Jejak Tani</h1>

          <div className="flex gap-2 mt-5">
            <button type="button" onClick={() => setRole('buyer')}
              className={`flex-1 btn-secondary !rounded-xl ${role === 'buyer' ? '!bg-teal-700 !text-white !border-teal-700' : ''}`}>
              Saya Pembeli
            </button>
            <button type="button" onClick={() => setRole('petani')}
              className={`flex-1 btn-secondary !rounded-xl ${role === 'petani' ? '!bg-teal-700 !text-white !border-teal-700' : ''}`}>
              Saya Petani/UMKM
            </button>
          </div>
